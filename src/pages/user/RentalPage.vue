<template>
  ...
  <input id="default-range" type="range" value="50" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"> -->
      ...
      <div class="lg:col-span-9 md:col-span-8">
        ...
        <section v-else-if="rentalItemUpdated && !isMobile" class="relative">
          <div class="md:flex justify-between items-center mb-3">
            <span class="font-semibold">Showing {{ updatedItemsByRental.length }} items</span>
            <div v-if="updatedItemsByRental.length != 0" class="md:flex items-center"
              :class="[mobSorting ? 'flex' : 'hidden', isMobile ? 'bg-white fixed bottom-0 left-0 w-full z-[998] pt-4 px-4 border-t' : 'flex', isMobile ? (!mobile_app_info ? 'pb-[120px]' : 'pb-[66px]') : 'pb-0']">
              <label class="font-semibold md:me-2" :class="[mobSorting ? 'hidden' : '']">Sort by:</label>
              <select v-model="shortItem"
                class="form-select form-input text-sm md:w-48 w-full md:mt-0 mt-1 py-2 px-3 h-10 bg-transparent rounded outline-none border border-gray-200 focus:ring-0">
          ...
          <div v-if="updatedItemsByRental.length != 0">
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 gap-6">
              <ItemComponent v-for="x of updatedItemsByRental" :key="x" :item="x" :brand="x.brand"></ItemComponent>
            </div>
            <div class="center">
              <div class="flex flex-wrap my-6 justify-end">
                <a href="#"
...
const product = computed(() => {
  return {
    name: route?.query?.name !== undefined ? `${route?.query?.name?.charAt(0).toUpperCase()}${route?.query?.name?.slice(1)}` : 'Best Rental',
    subname: '',
    href: route?.fullPath,
    breadcrumbs: [
      { id: 1, name: 'Home', href: '#' },
  ...
  computed(() => {
    return gql`
      query  GetItemsByPageCount{
        itemsByPageCount(id: "${itemId.value}", amount: "${itemAmount.value}", less1: ${less1.value}, less6: ${less6.value}, greater6: ${great6.value} categorytypeid: "${categoryTypeId.value}",  name: "rental")
      }
     `
  })
)
...
watchEffect(() => {
  ...
  let itemFilter = [...(rentalitems.value?.itemsByRental || [])];
...
const updatedItemsByRental = computed(() => {
  let itemFilter = [...(rentalitems.value?.itemsByRental || [])];
  switch (shortItem.value) {
    case "Price: Low-High":
      itemFilter.sort((a, b) => ((a.mrp * 5) / 100) - ((b.mrp * 5) / 100));
      break;
    case "Price: High-Low":
      itemFilter.sort((a, b) => ((b.mrp * 5) / 100) - ((a.mrp * 5) / 100));
      break;
    default:
...
const updatedMobileItemsByRental = computed(() => {
