  ...
  <div id="itemsSection" class="mx-auto w-full 2xl:container bg-white pb-8 px-4">
    ...
    <div class="grid md:grid-cols-12 grid-cols-1 gap-6">
      ...
      <div class="lg:col-span-9 md:col-span-8">
        <section v-if="filterDataUpdate && !isMobile" class="relative">
          <div class="md:flex justify-between items-center mb-3">
            ...
            <div v-if="updatedItemsFiltersData.length != 0" class="md:flex items-center"
              :class="[mobSorting ? 'flex' : 'hidden', isMobile ? 'bg-white fixed bottom-0 left-0 w-full z-[998] pt-4 px-4 pb-[66px] border-t' : 'flex', isMobile ? (!mobile_app_info ? 'pb-[120px]' : 'pb-[66px]') : 'pb-0']">
        ...
        <section v-else-if="bestItemSellingUpdated && !isMobile" class="relative md:pl-4">
          <div class="md:flex justify-between items-center mb-6">
            <span class="font-semibold">Showing {{ updatedItemsByBestSellData.length }} items</span>
            <div v-if="updatedItemsByBestSellData.length != 0" class="md:flex items-center"
              :class="[mobSorting ? 'flex' : 'hidden', isMobile ? 'bg-white fixed bottom-0 left-0 w-full z-[998] pt-4 px-4 pb-[66px] border-t' : 'flex', isMobile ? (!mobile_app_info ? 'pb-[120px]' : 'pb-[66px]') : 'pb-0']">
              <label class="font-semibold md:me-2" :class="[mobSorting ? 'hidden' : '']">Sort by:</label>
              <select v-model="shortItem"
                class="form-select form-input text-sm md:w-48 w-full md:mt-0 mt-1 py-2 px-3 h-10 bg-transparent rounded outline-none border border-gray-200 focus:ring-0">
          ...
          <div v-if="updatedItemsByBestSellData.length != 0">
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 gap-6">
              <ItemComponent v-for="x of updatedItemsByBestSellData" :key="x" :item="x" :brand="x.brand">
              </ItemComponent>
            </div>
            <div class="center">
              <div class="flex flex-wrap my-6 justify-end">
        ...
        <section v-else-if="bestItemSellingUpdated && isMobile" class="relative md:pl-4">
          <div class="md:flex justify-between items-center mb-6">
            <span class="font-semibold">Showing {{ updatedMobileItemsByBestSellData.length }} items</span>
            <div v-if="updatedMobileItemsByBestSellData.length != 0" class="md:flex items-center"
              :class="[mobSorting ? 'flex' : 'hidden', isMobile ? 'bg-white fixed bottom-0 left-0 w-full z-[998] pt-4 px-4 pb-[66px] border-t' : 'flex', isMobile ? (!mobile_app_info ? 'pb-[120px]' : 'pb-[66px]') : 'pb-0']">
              <label class="font-semibold md:me-2" :class="[mobSorting ? 'hidden' : '']">Sort by:</label>
              <select v-model="shortMobileItem"
                class="form-select form-input text-sm md:w-48 w-full md:mt-0 mt-1 py-2 px-3 h-10 bg-transparent rounded outline-none border border-gray-200 focus:ring-0">
          ...
          <div v-if="updatedMobileItemsByBestSellData.length != 0">
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 gap-6">
              <ItemComponent v-for="x of updatedMobileItemsByBestSellData" :key="x" :item="x" :brand="x.brand">
              </ItemComponent>
            </div>
            <div v-if="visiblePages.length > pageNo + 1" class="mt-6 flex justify-center">
              <button @click="loadMore"
  ...
  computed(() => {
    return gql`
      query  GetItemsByPageCount{
        itemsByPageCount(id: "${itemId.value}", amount: "${itemAmount.value}", less1: ${less1.value}, less6: ${less6.value}, greater6: ${great6.value} categorytypeid: "${categoryTypeId.value}",  name: "bestselling")
      }
     `
  })
)
...
const updatedMobileItemsByBestSellData= computed(() => {
