      ...
      <div class="lg:col-span-9 md:col-span-8">
        ...
        <section v-else-if="filterDataUpdate && isMobile" class="relative">
          ...
          <div v-if="updatedItemsMobileFiltersData.length > 0">
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 gap-6 pt-3">
              <ItemComponent v-for="x of updatedItemsMobileFiltersData" :key="x" :item="x" :brand="x.brand"></ItemComponent>
            </div>
        ...
        <section v-else-if="ageUpdated && !isMobile" class="relative">
          <div class="md:flex justify-between items-center mb-3">
            <span class="font-semibold">Showing {{ updatedAgeFiltersData.length }} items</span>
            <div v-if="updatedAgeFiltersData.length != 0" class="md:flex items-center"
              :class="[mobSorting ? 'flex' : 'hidden', isMobile ? 'bg-white fixed bottom-0 left-0 w-full z-[998] pt-4 px-4 border-t' : 'flex', isMobile ? (!mobile_app_info ? 'pb-[120px]' : 'pb-[66px]') : 'pb-0']">
              <label class="font-semibold md:me-2" :class="[mobSorting ? 'hidden' : '']">Sort by:</label>
              <select v-model="shortItem"
                class="form-select form-input text-sm md:w-48 w-full md:mt-0 mt-1 py-2 px-3 h-10 bg-transparent rounded outline-none border border-gray-200 focus:ring-0">
          ...
          <div v-if="updatedAgeFiltersData.length != 0">
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 gap-6 pt-3">
              <ItemComponent v-for="x of updatedAgeFiltersData" :key="x" :item="x" :brand="x.brand"></ItemComponent>
            </div>
            <div class="center">
              <div class="flex flex-wrap my-6 justify-end">
                <a href="#"
        ...
        <section v-else-if="ageUpdated && isMobile" class="relative">
          <div class="md:flex justify-between items-center mb-3">
            <span class="font-semibold">Showing {{ updatedAgeMobileFiltersData.length }} items</span>
            <div v-if="updatedAgeMobileFiltersData.length != 0" class="md:flex items-center"
              :class="[mobSorting ? 'flex' : 'hidden', isMobile ? 'bg-white fixed bottom-0 left-0 w-full z-[998] pt-4 px-4 border-t' : 'flex', isMobile ? (!mobile_app_info ? 'pb-[120px]' : 'pb-[66px]') : 'pb-0']">
              <label class="font-semibold md:me-2" :class="[mobSorting ? 'hidden' : '']">Sort by:</label>
              <select v-model="shortMobileItem"
                class="form-select form-input text-sm md:w-48 w-full md:mt-0 mt-1 py-2 px-3 h-10 bg-transparent rounded outline-none border border-gray-200 focus:ring-0">
          ...
          <div v-if="updatedAgeMobileFiltersData.length != 0">
            <div class="grid xl:grid-cols-4 lg:grid-cols-3 grid-cols-2 gap-6 pt-3">
              <ItemComponent v-for="x of updatedAgeMobileFiltersData" :key="x" :item="x" :brand="x.brand"></ItemComponent>
            </div>
            <div v-if="visiblePages.length > pageNo + 1" class="mt-6 flex justify-center">
              <button @click="loadMore"
                class="text-gray-900 border border-sm  rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none ">
...
const product = computed(() => {
  return {
    name: route?.params?.age !== undefined ? `${route?.params?.age?.charAt(0).toUpperCase()}` : 'Best Offers',
    subname: '',
    href: route?.fullPath,
    breadcrumbs: [
      { id: 1, name: 'Home', href: '#' },
  ...
  computed(() => {
    return gql`
      query  GetItemsByPageCount{
        itemsByPageCount(id: "${itemId.value}", amount: "${itemAmount.value}", less1: ${less1.value}, less6: ${less6.value}, greater6: ${great6.value} categorytypeid: "${categoryTypeId.value}",  name: "age")
      }
     `
  })
)
...
watchEffect(() => {
  ...
  let itemFilter = [...(itemsByAge.value?.itemsByAge || [])];
...
const updatedAgeFiltersData = computed(() => {
  let itemFilterAgeData = [...(itemsByAge.value?.itemsByAge || [])];
  switch (shortItem.value) {
    case "Price: Low-High":
...
const updatedAgeMobileFiltersData = computed(() => {
