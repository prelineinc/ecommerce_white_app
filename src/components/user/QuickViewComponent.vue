                    ...
                    <div class="sm:col-span-8 lg:col-span-7">
                      ...
                      <section aria-labelledby="information-heading" class="mt-2">
                        ...
                        <div class="mt-6">
                          ...
                          <div class="flex items-center">
                            ...
                            <a href="#" class="ml-3 text-sm font-medium text-blue-600 hover:text-blue-500">{{ product.reviewCount }} reviews</a>
                          </div>
                        </div>
                      </section>
                      ...
                      <section aria-labelledby="options-heading" class="mt-10">
                        ...
                        <form>
                            ...
                            <RadioGroup v-model="selectedSize" class="mt-4">
                              ...
                              <div class="grid grid-cols-4 gap-4">
                                <RadioGroupOption as="template" v-for="size in product.sizes" :key="size.name" :value="size" :disabled="!size.inStock" v-slot="{ active, checked }">
                                  <div :class="[size.inStock ? 'cursor-pointer bg-white text-gray-900 shadow-sm' : 'cursor-not-allowed bg-gray-50 text-gray-200', active ? 'ring-2 ring-blue-500' : '', 'group relative flex items-center justify-center rounded-md border py-3 px-4 text-sm font-medium uppercase hover:bg-gray-50 focus:outline-none sm:flex-1']">
                                    <RadioGroupLabel as="span">{{ size.name }}</RadioGroupLabel>
                                    <span v-if="size.inStock" :class="[active ? 'border' : 'border-2', checked ? 'border-blue-500' : 'border-transparent', 'pointer-events-none absolute -inset-px rounded-md']" aria-hidden="true" />
                          ...
                          <button type="submit" class="mt-6 flex w-full items-center justify-center rounded-md border border-transparent text-yellow-900 bg-yellow-400 hover:bg-yellow-300 px-8 py-3 text-base font-medium focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:ring-offset-2">Add to bag</button>
                        </form>
                      </section>
                    </div>
                  </div>
